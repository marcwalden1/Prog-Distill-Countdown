...
if [ -n "${FINAL_JID:-}" ]; then
    if [ -z "${TRAIN_JID:-}" ]; then
        ...
        EVAL_JID1=$(EVAL_EXTRA_ARGS="${EVAL_EXTRA_ARGS:-}" EVAL_DATASET=balanced \
            ...
            sbatch --parsable \
                ...
                ${EVAL_SBATCH_ARGS} \
                scripts/eval.sh)
        EVAL_JID2=$(EVAL_EXTRA_ARGS="${EVAL_EXTRA_ARGS:-}" EVAL_DATASET=balanced5 \
            EVAL_CHECKPOINT_PATH="${FINAL_EVAL_CKPT}" \
            EVAL_RESULT_PATH="${eval_result_base}" \
            sbatch --parsable \
                --partition=$_eval_partition --account=$_eval_account \
                ...
                ${EVAL_SBATCH_ARGS} \
                scripts/eval.sh)
        EVAL_JID3=$(EVAL_EXTRA_ARGS="${EVAL_EXTRA_ARGS:-}" EVAL_DATASET=balanced6 \
            EVAL_CHECKPOINT_PATH="${FINAL_EVAL_CKPT}" \
            EVAL_RESULT_PATH="${eval_result_base}" \
            sbatch --parsable \
                --partition=$_eval_partition --account=$_eval_account \
        ...
        EVAL_JID1=$(EVAL_EXTRA_ARGS="${EVAL_EXTRA_ARGS:-}" EVAL_DATASET=balanced  sbatch --parsable \
            --partition=$_eval_partition --account=$_eval_account \
            --dependency=afterok:${FINAL_JID} \
            ${EVAL_SBATCH_ARGS} \
            scripts/eval.sh)
        EVAL_JID2=$(EVAL_EXTRA_ARGS="${EVAL_EXTRA_ARGS:-}" EVAL_DATASET=balanced5 sbatch --parsable \
            --partition=$_eval_partition --account=$_eval_account \
            --dependency=afterok:${FINAL_JID} \
            ${EVAL_SBATCH_ARGS} \
            scripts/eval.sh)
        EVAL_JID3=$(EVAL_EXTRA_ARGS="${EVAL_EXTRA_ARGS:-}" EVAL_DATASET=balanced6 sbatch --parsable \
            --partition=$_eval_partition --account=$_eval_account \
            --dependency=afterok:${FINAL_JID} \
            ${EVAL_SBATCH_ARGS} \
            scripts/eval.sh)
